    ...
    def run(self):
        ...
        if raw_data:
            ...
            for stat in root.findall('stat'):
                name = None
                if stat.find('type') is not None:
                    name = '{}.{}.{}'.format(
                        stat.find('type').text,
                        stat.find('ident').text,
                        stat.find('name').text
                    )
                else:
                ...
                if name:
                    data[name] = int(stat.find('value').text)
            ...
            metrics = {
                'client stats': {
                    'connection': data['client_conn'],
                    'drop': data['client_drop'],
                    'request': data['client_req'],
                },
                'cache stats': {
                    'hits': data['cache_hit'],
                    'misses': data['cache_miss']
                },
                'backend connection stats': {
                    'success': data['backend_conn'],
                    'not attempted': data['backend_unhealthy'],
                    'failures': data['backend_fail'],
                    'closed': data['backend_toolate']
                },
            }
